...
func init() {
	...
	singleRegister("unbzip2", bunzip2, withDescription("decompress bzip2 data in parallel (X:0 is concurrency, 0 is one worker per CPU)"),
		withCategory("decompress"),
		withConfigBuilder(stdConfigUint64WithDefault(0)),
		withAliases("punbzip2"),
	)
}
...
// bunzip2 decodes bzip2 blocks concurrently; about 5x faster than
// compress/bzip2 on 4 cores, and it handles concatenated streams too.
func bunzip2(w io.WriteCloser, r io.ReadCloser, config any) (int64, error) {
